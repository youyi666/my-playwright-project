...
(async () => {
    ...
    // ==========================================
    // [增量模块] 读取本地 txt 文件建立任务队列
    // ==========================================
    const txtPath = path.join(__dirname, 'goods_ids.txt');
    if (!fs.existsSync(txtPath)) {
        console.error(`❌ [环境阻断] 找不到文件 ${txtPath}，请在项目根目录下创建该文件，并每行填入一个商品ID。`);
        return; // 终止执行
    }
    const rawData = fs.readFileSync(txtPath, 'utf-8');
    // 清洗数据：按换行符分割，去除首尾空格，过滤掉空行
    const targetIds = rawData.split(/\r?\n/).map(id => id.trim()).filter(id => id !== '');
    
    if (targetIds.length === 0) {
        console.error(`⚠️ [环境阻断] goods_ids.txt 文件为空，请输入商品ID后再运行。`);
        return;
    }
    console.log(`📂 [文件读取] 成功读取商品列表，共计 ${targetIds.length} 个商品待处理。`);
    // ==========================================
    ...
    for (let i = 0; i < targetIds.length; i++) {
        const currentProductId = targetIds[i];
        console.log(`\n===========================================`);
        console.log(`▶️ [循环进度] 正在处理第 ${i + 1}/${targetIds.length} 个商品，当前目标ID: ${currentProductId}`);
        console.log(`===========================================\n`);
        ...
        try {
            ...
            // [增量模块] 将原本写死的 ID 替换为循环变量
            await inputA.fill(currentProductId);
            ...
            if (!productA_Data) {
                console.log(`❌ 严重警告：没有抓到 A 店数据。将跳过此商品 ${currentProductId}`);
                // [增量模块] 若未抓到数据，关闭页面进入下一轮循环
                await editPageA.close();
                await pageA.close();
                continue; 
            }
            ...
            await inputB.fill('925200242628');
            ...
            try {
                ...
                console.log(`\n🎊 [单品完成] 商品 ${currentProductId} 自动化流程执行完毕！`);

                // [增量模块] 清理资源，关闭当前循环打开的页面，防止越开越多卡死
                await editPageB.waitForTimeout(2000); // 留出请求发送的缓冲时间
                await editPageB.close();
                await pageB.close();
                await editPageA.close();
                await pageA.close();
            ...
            console.error(`❌ [单品异常] 处理商品 ${currentProductId} 时运行出错:`, error.message);
            // [增量模块] UI 交互错误截图保存逻辑，防止程序直接崩溃并留底排查
            try {
                const errorPicName = `Error_${currentProductId}_${Date.now()}.png`;
                const pagesA = contextA.pages();
                if (pagesA.length > 0) {
                    await pagesA[pagesA.length - 1].screenshot({ path: path.join(__dirname, errorPicName) });
                    console.log(`📸 已保存错误现场截图至项目根目录: ${errorPicName}`);
                }
            } catch (picErr) {
                console.log('⚠️ 尝试保存错误截图失败。');
            }
        }
    } // 结束循环

    console.log('\n>>> 🏁 所有任务列表均已跑完！浏览器将保持开启状态供你检查。按 Ctrl+C 结束。');
