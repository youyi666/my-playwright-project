// 04-PDD_Activity_Monitor.js - 解决抓取不到数据的诊断版本

const { chromium, errors } = require('playwright');
const xlsx = require('xlsx');
const path = require('path');
const fs = require('fs/promises'); // 使用 fs/promises 以简化文件操作
...
// [路径改动 2: 输入Excel路径] - 相对路径
const INPUT_EXCEL_PATH = path.join(__dirname, 'exc_data', '映射_平台商品', 'products.xlsx'); 
const INPUT_SHEET_NAME = 'products1';
const PRODUCT_ID_COLUMN_HEADER = '商品ID';
// [路径改动 3: CSV 输出目录] - 相对路径
const OUTPUT_DIR = path.join(__dirname, 'exc_data', '活动_营销活动'); 
// [路径改动 4: 数据库路径] - 相对路径
const DB_PATH = path.join(__dirname, 'sql_data', 'pdd_activity.db'); 
...
async function saveResultsToCsv(data, filePath) {
    ...
    console.log(`\n--- [保存结果] 准备将 ${data.length} 条记录保存到CSV文件: ${filePath} ---`); 
    try {
...
async function saveResultsToDb(data, dbPath) {
    ...
    try {
        ...
        console.error(`❌ [DB错误] 数据库连接失败: ${error.message}`);
        return;
    }
    ...
    db.serialize(() => {
        ...
        const columnDefs = Array.from(columnsSet).map(col => `"${col}" TEXT`).join(', ');
        ...
        db.run(createTableSql, (err) => {
            ...
            db.all(`PRAGMA table_info(activity)`, (err, tableInfo) => {
                if (err) {
                    console.error(`❌ [DB错误] 无法获取表结构信息: ${err.message}`);
                    return;
                }

                const existingColumns = new Set(tableInfo.map(col => col.name));
                const newColumns = Array.from(columnsSet).filter(col => !existingColumns.has(col));

                if (newColumns.length > 0) {
                    console.log(` -> 发现 ${newColumns.length} 个新列需要添加到表中: [${newColumns.join(', ')}]`);
                    db.exec('BEGIN TRANSACTION;');
                    newColumns.forEach(newCol => {
                        const alterSql = `ALTER TABLE activity ADD COLUMN "${newCol}" TEXT`;
                        db.run(alterSql, (err) => {
                            if (err) {
                                console.error(`❌ [DB错误] 添加列 "${newCol}" 失败: ${err.message}`);
                            }
                        });
                    });
                    db.exec('COMMIT;', (err) => {
                        if (err) {
                            console.error(`❌ [DB错误] 提交事务失败: ${err.message}`);
                        } else {
                            console.log(` -> 成功添加 ${newColumns.length} 个新列。`);
                        }
                        executeInsert(db, columns, data); 
                    });
                } else {
                    executeInsert(db, columns, data); 
                }
        ...
        function executeInsert(dbInstance, finalColumns, records) {
            const placeholders = finalColumns.map(() => '?').join(', ');
            const insertSql = `INSERT OR REPLACE INTO activity (${finalColumns.map(col => `"${col}"`).join(', ')}) VALUES (${placeholders})`;
            
            const stmt = dbInstance.prepare(insertSql, (err) => {
                if (err) {
                    console.error(`❌ [DB错误] 准备插入语句失败: ${err.message}`);
                    dbInstance.close();
                    return;
                }
                ...
                for (const record of records) {
                    const values = finalColumns.map(col => record[col.trim()]); 
                    stmt.run(values, (err) => {
                        if (err) {
                            console.error(`❌ [DB错误] 插入记录失败 (ID: ${record[PRODUCT_ID_COLUMN_HEADER]}): ${err.message}`);
                        }
                    });
                }
                
                dbInstance.exec('COMMIT;', (err) => {
                    if (err) {
                        console.error(`❌ [DB错误] 提交事务时出错: ${err.message}`);
                    } else {
                        console.log(` -> ${records.length} 条记录已成功写入/更新。`);
                    }
                    stmt.finalize((err) => {
    ...
    for (const target of TARGETS) {
        ...
        for (const [index, product] of productsData.entries()) {
            ...
            try {
                ...
                // --- [新增诊断代码 - START] ---
                const recordContainerLocator = page.locator('.activity-table-row:visible, .table_row:visible'); 
                let hasVisibleRecords = false;
                try {
                    await recordContainerLocator.first().waitFor({ state: 'visible', timeout: 5000 });
                    const totalRecords = await recordContainerLocator.count();
                    console.log(` -> [诊断] 页面上找到 ${totalRecords} 个可见的活动记录容器。`);
                    if (totalRecords > 0) {
                        hasVisibleRecords = true;
                    }
                } catch (e) {
                    console.log(' -> [诊断] 未找到任何活动记录容器。');
                }
                // --- [新增诊断代码 - END] ---
